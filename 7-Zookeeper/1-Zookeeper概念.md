...
- 持久节点 - 即使在创建该特定的znode的客户端断开连接后,持久节点仍然存在。默认情况下,除非另有说明,否则所有znode都是持久的。
- 临时节点 - 客户端活跃时，临时节点就是有效的。当客户端与Zookeeper集合断开连接时，临时节点会自动删除，因此，只有临时节点不允许有子节点。如果临时节点被删除，则一个合适点节点将填充其位置。临时节点在Leader选举中起着重要作用
- 顺序节点 - 顺序节点可以是持久的或或临时的。当一个新的znode被创建一个为顺序节点时，Zookeeper通过将位数的序号附加到原始名称来设置znode的路径。例如，如果将具有路径`/myapp`的znode创建为顺序节点，则Zookeeper将路径更改为`/myapp0000000001`，并将下一个序号设置为`0000000002`。如果两个顺序节点是同时创建的，那么Zookeeper不会对每个znode使用相同的数字。顺序节点锁定和同步起重要作用。
